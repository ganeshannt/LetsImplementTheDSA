...
public class Java21Features {
    ...
    private static void demonstratePatternMatchingForSwitch() {
        ...
        Object[] objects = {
                ...
                null,              // null
                true,              // Boolean
                new Java17Features.Person("Alice", 30), // Custom record from earlier example
        ...
        Object nestedStructure = new Java17Features.Address("123 Main St", "Springfield", "IL", "62701");
        System.out.println(describeObject(nestedStructure));

        nestedStructure = new Java17Features.Person("Bob", 42);
        System.out.println(describeObject(nestedStructure));
    }
    ...
    private static String formatValue(Object obj) {
        return switch (obj) {
            ...
            case Java17Features.Person p -> "Person named " + p.name() + ", age " + p.age();
    ...
    private static String describeObject(Object obj) {
        return switch (obj) {
            case Java17Features.Address address -> {
                String city = address.city();
                String state = address.state();
                String zip = address.zipCode();
                yield "Address in " + city + ", " + state + " with zip " + zip;
            }
            case Java17Features.Person person -> {
                String name = person.name();
                Integer age = person.age();
                if (age >= 18) {
                    yield "Adult named " + name;
                } else {
                    yield "Minor named " + name;
                }
            }
